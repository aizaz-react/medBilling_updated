...
export default function Example() {
  ...
  return (
    <header className="bg-white z-10 sticky top-0">
      <nav
        className="font-Lato flex flex-row justify-between items-center py-2 px-12 shadow-sm"
        aria-label="Global"
      >
        <Link to="/" alt="logo">
  ...
  return (
    <div className="invisible group-hover:visible hover:visible md:block hidden max-w-[900px] absolute top-14 duration-100 ease-in-out inset-0 md:mx-auto my-auto mx-6">
      <div className="bg-white border border-gray-300 px-6 flex flex-col gap-4">
        <h1 className="font-Oswald font-bold text-2xl text-gray-700 py-4 border-b-[1px] border-gray-300">
          Our Services
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((item, index) => (
            <Link to={item.link} key={index}>
              <h1 className="font-Oswald flex flex-col gap-2 text-left mb-2 text-xl font-medium leading-tight text-neutral-800 dark:text-neutral-50 w-max">
                {item.title}
